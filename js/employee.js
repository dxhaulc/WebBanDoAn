...
document.addEventListener('DOMContentLoaded', function() {
    // document.querySelector('.header__navbar-item-link-manage').onclick = function () {
        ...
        fetch('http://localhost:3000/nhan-vien')
            ...
            .then(data => {
                manages = data.map(manage => ({
                    id: manage.IDNV,
                    name: manage.HoTen,
                    birthday: manage.NgaySinh,
                    title: manage.ChucVu,
                    salary: manage.Luong,
                    phonenumber: manage.SDT,
                    account_name: manage.TaiKhoanNV
                }));
        ...
        document.getElementById("btn-search").onclick = function () {
            ...
            fetch(`http://localhost:3000/nhan-vien/search?${query}`)
                .then(response => response.json())
                .then(data => {
                    manages = data.map(manage => ({
                        id: manage.IDNV,
...
function paginateManage(data, page = 1, recordsPerPage = 2) {
    ...
    listing_table.innerHTML = `
        <tr>
            ...
            <th>SĐT</th>
            <th>Tên TK</th>
            <th><button>Thêm nhân viên</button></th>
        </tr>`;
    ...
    pageData.forEach(manage => {
        ...
        row.insertCell(2).innerHTML = new Date(manage.birthday).toISOString().split("T")[0];
        row.insertCell(3).innerHTML = manage.title;
        row.insertCell(4).innerHTML = manage.salary;
        row.insertCell(5).innerHTML = manage.phonenumber;
        row.insertCell(6).innerHTML = manage.account_name;
...
function openManageForm(manage = null) {
    ...
    document.getElementById('manage-salary').value = manage ? manage.salary : '';
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('save-manage-btn').onclick = function() {
        const manageData = {
            ...
            salary: document.getElementById('manage-salary').value,
        ...
        if (editingManage) {
            fetch(`http://localhost:3000/nhan-vien/${editingManage.id}`, {
                method: 'PUT',
...
function deleteManage(id) {
    if (confirm("Bạn có chắc chắn muốn xóa nhân viên này?")) {
        fetch(`http://localhost:3000/nhan-vien/${id}`, {
            method: 'DELETE'
        })
        .then(response => {
            if (response.ok) {
